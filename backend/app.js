...
async function selectSubject(id) {
    ...
    loading(true);
    try {
        // Check if a default class is assigned for this subject
        const defaultRes = await api(`/teacher/default-class?subjectId=${id}`);

        if (defaultRes.found) {
            // Use default class — skip class selection step
            selectedClass = defaultRes.defaultClass.class_id;
            document.getElementById('step4').classList.add('completed');
            document.getElementById('step5').classList.add('completed');
            document.getElementById('step6').classList.add('active');
            document.getElementById('teacher-content').innerHTML = `
                <button class="btn btn-secondary" onclick="selectSemester(${selectedSemester})">← Back</button>
                <h3>Choose Mode</h3>
                <div class="default-class-banner">
                    <span>📍 Default Class: <strong>${defaultRes.defaultClass.class_name}</strong> — ${defaultRes.defaultClass.building || ''}</span>
                    <button class="btn btn-secondary" onclick="pickClassManually()" style="padding:6px 12px;font-size:13px;">🔄 Switch Class</button>
                </div>
                <div class="selection-grid">
                    <div class="mode-card" onclick="selectMode('geo')">
                        <i class="fas fa-map-marker-alt"></i>
                        <h3>📍 Geo-Fenced</h3><p>Auto check-in</p>
                    </div>
                    <div class="mode-card" onclick="selectMode('manual')">
                        <i class="fas fa-hand-pointer"></i>
                        <h3>✍️ Manual</h3><p>Mark manually</p>
                    </div>
                </div>`;
        } else {
            // No default class — show class picker normally
            await pickClassManually();
        }
    } catch (error) {
        await pickClassManually();
    } finally {
...
async function pickClassManually() {
    loading(true);
    try {
        ...
        document.getElementById('teacher-content').innerHTML = `
            ...
            <h3>Select Class</h3>
            <div class="selection-grid">
                ${classes.map(c => `<div class="selection-card" onclick="selectClass(${c.id})"><h3>${c.name}</h3><p>${c.building}</p></div>`).join('')}
            </div>`;
    } catch (error) {
        alert('Error: ' + error.message);
    } finally {
...
function selectClass(id) {
    ...
    document.getElementById('step6').classList.add('active');
    document.getElementById('teacher-content').innerHTML = `
        <button class="btn btn-secondary" onclick="pickClassManually()">← Back</button>
        <h3>Choose Mode</h3>
        <div class="selection-grid">
            <div class="mode-card" onclick="selectMode('geo')">
                <i class="fas fa-map-marker-alt"></i>
