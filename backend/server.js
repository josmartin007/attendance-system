...
app.post('/api/teacher/geo-session/start', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Send push notifications to all students in this session
        const [subjectInfo] = await pool.query('SELECT name FROM subjects WHERE id = ?', [subjectId]);
        const [teacherInfo] = await pool.query('SELECT name FROM users WHERE id = ?', [teacherId]);
        const subjectName = subjectInfo[0]?.name || 'Unknown Subject';
        const teacherName = teacherInfo[0]?.name || 'Your Teacher';

        const pushPayload = {
            title: '📍 Attendance Session Started!',
            body: `${teacherName} has started attendance for ${subjectName} in ${classInfo[0].name}`,
            tag: `session-${sessionId}`,
            sessionId,
            url: '/index.html'
        };

        // Notify all students asynchronously (don't block response)
        notifySessionStudents(session, pushPayload).catch(console.error);
...
app.post('/api/admin/subjects', authenticateToken, async (req, res) => {
    const { name, code, department_id, semester, teacher_ids, default_class_id } = req.body;
    try {
        const [result] = await pool.query(
        ...
        if (teacher_ids && teacher_ids.length > 0) {
            for (let teacherId of teacher_ids) {
                ...
                // Save default class for each teacher
                if (default_class_id) {
                    await pool.query(
                        `INSERT INTO teacher_default_classes (teacher_id, subject_id, class_id)
                         VALUES (?, ?, ?)
                         ON DUPLICATE KEY UPDATE class_id = VALUES(class_id)`,
                        [teacherId, subjectId, default_class_id]
                    );
                }
            }
        }
        res.json({ success: true, insertId: subjectId });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
        ...
        if (teacher_ids) {
            ...
            if (teacher_ids.length > 0) {
                for (let teacherId of teacher_ids) {
                    ...
                    // Update default class for each teacher
                    if (default_class_id) {
                        await pool.query(
                            `INSERT INTO teacher_default_classes (teacher_id, subject_id, class_id)
                             VALUES (?, ?, ?)
                             ON DUPLICATE KEY UPDATE class_id = VALUES(class_id)`,
                            [teacherId, id, default_class_id]
                        );
                    }
                }
            }
            // If no default class, remove existing defaults for this subject
            if (!default_class_id) {
                await pool.query('DELETE FROM teacher_default_classes WHERE subject_id = ?', [id]);
            }
        }
...
// ================== DEFAULT CLASS ENDPOINTS ==================

// Admin: Get all teacher default classes
app.get('/api/admin/teacher-default-classes', authenticateToken, async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT tdc.id, tdc.teacher_id, tdc.subject_id, tdc.class_id,
                   u.name as teacher_name,
                   s.name as subject_name, s.code as subject_code,
                   c.name as class_name, c.building, c.room_number
            FROM teacher_default_classes tdc
            JOIN users u ON tdc.teacher_id = u.id
            JOIN subjects s ON tdc.subject_id = s.id
            JOIN classes c ON tdc.class_id = c.id
...
// Admin: Set/Update default class for a teacher-subject
app.post('/api/admin/teacher-default-classes', authenticateToken, async (req, res) => {
...
// Admin: Delete a default class assignment
app.delete('/api/admin/teacher-default-classes/:id', authenticateToken, async (req, res) => {
    try {
        await pool.query('DELETE FROM teacher_default_classes WHERE id = ?', [req.params.id]);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// Teacher: Get default class for a specific subject
app.get('/api/teacher/default-class', authenticateToken, async (req, res) => {
    const teacherId = req.user.id;
    const { subjectId } = req.query;
    try {
        const [rows] = await pool.query(`
            SELECT tdc.class_id, c.name as class_name, c.building, 
                   c.room_number, c.latitude, c.longitude, c.geo_radius
            FROM teacher_default_classes tdc
            JOIN classes c ON tdc.class_id = c.id
            WHERE tdc.teacher_id = ? AND tdc.subject_id = ?
        `, [teacherId, subjectId]);
        res.json(rows.length > 0 ? { found: true, defaultClass: rows[0] } : { found: false });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
// Then run: node -e "const wp=require('web-push'); const k=wp.generateVAPIDKeys(); console.log(k);"
// Paste the keys below

const VAPID_PUBLIC_KEY  = 'BBnJuDT0YFHUE3zGQjhGismWgsstrnZod-oi-6Lu78zTE0IznuSajI1z9trJHdiT_xike9xb3BhHhQeAQJp6LV8';
const VAPID_PRIVATE_KEY = 'ZYBQVdnxgr6cnZ8jJMrZEjKS2qlmzJl7dHwfZTUQzmk';
const VAPID_EMAIL       = 'mailto:admin@geoattend.com'; // change to your email

let webpush = null;
try {
    webpush = require('web-push');
    webpush.setVapidDetails(VAPID_EMAIL, VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY);
    console.log('✅ web-push loaded — push notifications enabled');
} catch(e) {
    console.warn('⚠️  web-push not installed. Run: npm install web-push');
    console.warn('   Push notifications will be disabled until installed.');
}

// In-memory push subscription store
// In production, save these to your database instead
const pushSubscriptions = new Map(); // userId -> subscription

// Student/Teacher: Save push subscription
...
// Student/Teacher: Remove push subscription
...
async function sendPushToUser(userId, payload) {
    ...
    try {
        ...
        if (error.statusCode === 410 || error.statusCode === 404) {
            ...
            console.error(`Push error for user ${userId}:`, error.message);
        }
    }
}

// Internal helper: send push to all students in a session
async function notifySessionStudents(session, payload) {
    if (!webpush) return;
    const promises = session.students.map(s => sendPushToUser(s.id, payload));
